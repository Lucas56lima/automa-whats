...
def envia_mensagem(diretorio,mensagem_inserida):
    ...
    for i, valida in enumerate(contatos_df['já enviado']):
        if valida != "v":
            ...
            mensagem_inserida = urllib.parse.quote(f"{pessoa},{mensagem_inserida}")
            link = f"https://web.whatsapp.com/send?phone={numero}&text={mensagem_inserida}"
            navegador.get(link)
        else:
            print("Mensagens já enviadas para contatos selecionados.")
...
mensagem_inserida = Text(janela)
mensagem_inserida.place(x=10,y=150,width=300,height=100)


botao = Button(janela,default='normal', text="Enviar mensagem", command=lambda: envia_mensagem(upload_arquivo,mensagem_inserida))
